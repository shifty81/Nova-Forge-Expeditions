...
void ShipCapabilityRatingSystem::update(float /*delta_time*/) {
    auto entities = world_->getEntities<components::ShipCapabilityRating>();
    for (auto* entity : entities) {
        auto* scr = entity->getComponent<components::ShipCapabilityRating>();
        if (!scr || !scr->active || !scr->needs_recalculation) continue;
