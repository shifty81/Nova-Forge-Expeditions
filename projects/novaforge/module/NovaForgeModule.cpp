...
atlas::module::GameModuleDesc NovaForgeModule::Describe() const {
    return {"NovaForge", 1};
}

void NovaForgeModule::RegisterTypes(atlas::module::GameModuleContext& ctx) {
    // Register factions from the NovaForge universe
...
void NovaForgeModule::ConfigureReplication(atlas::module::GameModuleContext& ctx) {
...
void NovaForgeModule::ConfigureServerRules(atlas::module::GameModuleContext& ctx) {
...
void NovaForgeModule::OnStart(atlas::module::GameModuleContext& ctx) {
    (void)ctx;
    m_started = true;
}

void NovaForgeModule::OnTick(atlas::module::GameModuleContext& ctx, float dt) {
    (void)ctx;
    (void)dt;
    ++m_tickCount;
}

void NovaForgeModule::OnShutdown(atlas::module::GameModuleContext& ctx) {
    (void)ctx;
    m_factions.Clear();
    m_combat.Clear();
    m_economy.Clear();
...
extern "C" atlas::module::IGameModule* CreateGameModule() {
    return new novaforge::NovaForgeModule();
}
